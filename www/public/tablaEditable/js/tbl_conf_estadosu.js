...
//            var estado = grid.getValueAt(cell.rowIndex, 3);
            ...
            menu += '<li><a href="javascript:editarEstado(\'' + id + '\');" class="btn btn-sm dropdown-item" ><span class="bi bi-std-14 bi-pencil"></span> Modificar</a></li>';
            menu += '<li><a href="' + urlBase + '/mantenimientos/eliminarestadoe/' + id + '" class="btn btn-sm dropdown-item" ><span class="bi bi-std-14 bi-x-circle text-danger"></span> Borrar</a></li>';
            menu += '<li><a href="javascript:abrirModal(\'' + id + '\');" class="btn btn-sm dropdown-item" ><i class="bi bi-envelope"></i> Enviar a Correo</a></li>';
    ...
    grid.renderGrid("divtabla", "table table-hover testgrid table-responsive table-bordered  table-striped", "tablas");
...
function updatePaginator(grid, divId) {
    ...
    // "first" link
    var link = $("<a class='nobg' title='Ir al primero'>").html("<i class=' fal fa-angle-left'><i class=' fal fa-angle-left'> ");
    if (!grid.canGoBack())
    ...
    // "prev" link
    link = $("<a class='nobg' title='Ir al anterior'>").html("<i class=' fal fa-angle-left'> ");
    if (!grid.canGoBack())
        link.css({opacity: 0.4, filter: "alpha(opacity=40)"});
    else
        link.css("cursor", "pointer").click(function (event) {
    ...
    // "next" link
    link = $("<a class='nobg' title='Ir al siguiente'>").html(" <i class=' fal fa-angle-right'>");
    ...
    paginator.append(link);
    // "last" link
    link = $("<a class='nobg' title='Ir al último'>").html(" <i class=' fal fa-angle-right'><i class=' fal fa-angle-right'>");
    if (!grid.canGoForward())
        link.css({opacity: 0.4, filter: "alpha(opacity=40)"});
    else
        link.css("cursor", "pointer").click(function (event) {
