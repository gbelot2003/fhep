...
class TipoexamenController extends AbstractActionController {
...
//            exit;
            ...
            if ($tipoexamen_model->agregarNuevo($datosFormularios)) {
                ...
                $_SESSION['mnsError'] = array('titulo' => 'Error!', 'texto' => 'Ocurrió un error al guardar el tipo de examen, vuelva a intentarlo.');
...
//    /    INICIO PDF VOUCHER TIPOS DE EXAMEN DE LABORATORIO
    public function pdffichaTipoexamenAction() {
// if (!isset($_SESSION['auth']) || $_SESSION['rol']['nombre_permiso'] != '1') {
// return $this->redirect()->toUrl($this->getRequest()->getBaseUrl() . "/");
// }
        if ($this->getRequest()->isXmlHttpRequest()) {
//para AJAX
            ...
            $this->dbAdapter = $this->getServiceLocator()->get('Zend\Db\Adapter');
                    $Tipoexamen_model = new Tipoexamen($this->dbAdapter);
            $id_tipoexamen = $this->params()->fromRoute("id", null);

//RECUPERAR LA INFORMACION DEL REPORTE
            $info_tipoexamen = $Tipoexamen_model->getTipoexameninfo($id_tipoexamen);
...
// exit;
            ...
            if (!$info_tipoexamen) {
                $_SESSION['mnsAutoOK'] = array('titulo' => 'Tipo de Examen Guardado!', 'texto' => "Tipo de Examen no encontrado.");
                return $this->redirect()->toUrl($this->getRequest()->getBaseUrl() . "/citas");
            }
            $nombre_tipoexamen = $info_tipoexamen[''];
            $descripcion_tipoexamen = $info_tipoexamen[''];
            $estado_tipoexamen = $info_tipoexamen[''];
...
// set información del PDF
            ...
            $pdf->SetTitle('Ficha de Tipos de Examen');
            $pdf->SetSubject('Tipos de Examen');
...
//Contenido
            ...
            $contenidoHTML = "<br><div style=\"text-align: justify; font-family: lucida consola; font-size: 14px; line-height: 25px;\">"
                    ...
                    . "Nombre Tipo de Examen:<b> $nombre_tipoexamen </b>"
                    . "</div>"
                    . "<div style=\"text-align: justify; font-family:lucida console,monospace; font-size: 14px; line-height: 12px;\">"
                    . "Descripción Tipo de Examen:<b> $descripcion_tipoexamen </b>"
                    . "</div>"
                    . "<div style=\"text-align: justify; font-family:lucida console,monospace; font-size: 14px; line-height: 12px;\">"
                    . "Estado Tipo de Examen: <b> $estado_tipoexamen </b>"
                    . "</div>"
                    . "<div style=\"text-align: justify; font-family:lucida console,monospace; font-size: 14px; line-height: 12px;\">"
...
// Cerrar enviar documento PDF , la I (i)= lo muestra en pantalla D = lo descarga a la maquina del cliente
            $pdf->Output("FICHA TIPOS DE EXAMENES DE LABORATORIOS " . '.pdf', 'I');
            exit;
        }
    }
