...
class EditableGrid {
    ...
    public function getXML($rows = false, $customRowAttributes = false, $encodeCustomAttributes = false, $includeMetadata = true) {
        ...
        if ($includeMetadata) {
            ...
            foreach ($this->columns as $name => $info) {
                ...
                if (!$info['bar'])
                    $columnNode->setAttribute('bar', 'false');
        ...
        if ($this->paginator !== null) {
            ...
            foreach ($this->paginator as $key => $value) {
                $paginatorNode->setAttribute($key, $value);
            }
        }
        ...
        if ($rows) {
            ...
            if (!$fetchMethod) {
                foreach ($rows as $row) {
                    $dataNode->appendChild($this->getRowXML($DOMDocument, $row, $customRowAttributes, $encodeCustomAttributes));
                }
            } else {
    ...
    private function getRowXML($DOMDocument, $row, $customRowAttributes, $encodeCustomAttributes) {
        ...
        if ($customRowAttributes)
            foreach ($customRowAttributes as $name => $field)
                $rowNode->setAttribute($name, $encodeCustomAttributes ? base64_encode($this->_getRowField($row, $field)) : $this->_getRowField($row, $field));
    ...
    public static function mapToArray($map) {
        ...
        foreach ($map as $k => $v) {
            if (is_array($v))
                $array[] = array('label' => $k, 'values' => self::mapToArray($v));
            else
    ...
    public function getPOJO($rows = false, $customRowAttributes = false, $encodeCustomAttributes = false, $includeMetadata = true) {
        ...
        if ($this->paginator !== null)
            $results['paginator'] = $this->paginator;
        ...
        if ($rows) {
            ...
            if (!$fetchMethod)
                foreach ($rows as $row)
                    $results['data'][] = $this->getRowPOJO($row, $customRowAttributes, $encodeCustomAttributes);
            else
                while ($row = call_user_func(array($rows, $fetchMethod)))
                    $results['data'][] = $this->getRowPOJO($row, $customRowAttributes, $encodeCustomAttributes);
        }
